...
from tools.mfw_install import build_from_base, build_install, prepare_output, safe_extract
...
def _assert_startup_payload(candidate: Path) -> None:
    ...
    assert tasks["GAME_START"]["entry"] == "0023-启动-游戏入口"
    assert tasks["GAME_START"]["default"] is True
    assert tasks["GAME_STOP"]["entry"] == "0024-启动-游戏停止"
...
def repo_fixture(tmp_path: Path) -> Path:
    ...
    (root / "assets/tasks/游戏启动.json").write_text(
        json.dumps(
            {
                "task": [
                    {"name": "GAME_START", "entry": "0023-启动-游戏入口", "default": True},
                    {"name": "GAME_STOP", "entry": "0024-启动-游戏停止", "default": False},
                ]
            }
        )
        + "\n",
    ...
    (root / "assets/resource/base/pipeline/startup/game_start.json").write_text(
        json.dumps(
            {
                "0023-启动-游戏入口": {"next": ["1356-启动-游戏启动"]},
                "1356-启动-游戏启动": {
    ...
    (root / "assets/resource/base/pipeline/startup/game_stop.json").write_text(
        '{"0024-启动-游戏停止":{"action":"StopApp","package":"com.hanjiasongshu.dr22"}}\n',
        encoding="utf-8",
    )

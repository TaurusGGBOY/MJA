...
def test_order_and_abort_helpers_cover_phase_boundaries() -> None:
    nodes = {
        ...
        "ABORT": {"action": "DoNothing", "next": ["1366-公共-通用中止"]},
        "1366-公共-通用中止": {"action": "StopTask"},
        "FAIL": {
            "action": "Custom",
    ...
    assert_native_failure_node(nodes["FAIL"])
...
def test_loop_exhaustion_requires_a_native_failure_leaf() -> None:
    nodes = {
    ...
    assert_loop_bound(nodes, "LOOP", 3)
    assert_native_failure_node(nodes["LOOP_EXHAUSTED"])
    nodes["LOOP_EXHAUSTED"] = {"action": "StopTask"}
    with pytest.raises(AssertionError):
        assert_native_failure_node(nodes["LOOP_EXHAUSTED"])
...
def test_battle_result_partition_rejects_missing_unknown_abort_branch() -> None:
    ...
    nodes["MJA_RESULT_UNKNOWN_RESULT"]["next"] = ["FAIL"]
    assert_battle_result_partition(nodes, "MJA_RESULT")

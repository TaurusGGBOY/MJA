...
def test_shop_has_explicit_available_and_already_claimed_candidates() -> None:
    ...
    assert "1244-商店免费礼包-打开-周期-福利" in pipeline
    assert pipeline["1244-商店免费礼包-打开-周期-福利"]["next"] == [
        "1239-商店免费礼包-商店-周期-权益-页面",
    ]
    assert pipeline["1239-商店免费礼包-商店-周期-权益-页面"]["next"] == [
        "1230-商店免费礼包-已领取",
        "1232-商店免费礼包-领取",
    ]
    assert pipeline["商店免费礼包-免费卡已领取文字"]["expected"] == [
        "已领取",
        "今日已领取",
        "领取完毕",
    ]
    assert pipeline["1230-商店免费礼包-已领取"]["next"] == ["1231-商店免费礼包-已完成"]
...
def test_shop_success_candidates_converge_on_native_cleanup() -> None:
    ...
    assert pipeline["1235-商店免费礼包-关闭"]["next"] == ["1236-商店免费礼包-关闭-面板"]
    assert pipeline["1236-商店免费礼包-关闭-面板"]["next"] == ["1371-公共-原生成功-主页边界"]
    assert nodes["1372-公共-原生成功-尝试返回"]["custom_action"] == "ReturnToWorldHome"
    assert_native_success_node(nodes["1369-公共-通用停止"])
    assert_reachable(nodes, "1231-商店免费礼包-已完成", "1372-公共-原生成功-尝试返回")
    assert_reachable(nodes, "1234-商店免费礼包-成功", "1372-公共-原生成功-尝试返回")
...
def test_shop_rechecks_claim_evidence_and_requires_home_cleanup() -> None:
    pipeline = _pipeline()

    assert pipeline["1233-商店免费礼包-关闭-奖励"]["on_error"] == ["1230-商店免费礼包-已领取"]
    assert pipeline["1235-商店免费礼包-关闭"]["on_error"] == ["1372-公共-原生成功-尝试返回"]
    assert pipeline["1236-商店免费礼包-关闭-面板"]["on_error"] == ["1372-公共-原生成功-尝试返回"]
...
def test_shop_preserves_navigation_evidence_and_action_bounds() -> None:
    ...
    assert pipeline["0019-商店免费礼包-任务入口"]["next"] == ["1227-商店免费礼包-打开-面板"]
    assert pipeline["1227-商店免费礼包-打开-面板"]["next"] == ["1228-商店免费礼包-打开-商店"]
    assert pipeline["1228-商店免费礼包-打开-商店"]["next"] == ["1229-商店免费礼包-商店-页面"]

...
def test_collection_entry_reads_the_bottom_left_collection_deployment_label() -> None:
    node = _load("daily/collection_deployment_daily.json")["0243-采集部署-采集-打开"]
...
def test_collection_deployment_button_reads_the_bottom_left_one_click_button() -> None:
    node = _load("daily/collection_deployment_daily.json")["0246-采集部署-采集-部署-全部"]
...
def test_food_and_equipment_share_the_bottom_right_item_entry() -> None:
    food = _load("daily/eat_stamina_food_daily.json")["0413-吃体力食物-食物-资源-入口"]
    equipment = _load("daily/equipment_decompose_daily.json")["0457-分解装备-装备-资源-入口"]
    expected = {
        "recognition": "OCR",
        "expected": "(?:道)?具",
        "roi": [980, 580, 300, 140],
...
def test_daily_entry_uses_the_current_bottom_right_trial_marker() -> None:
    node = _load("daily/daily_task_reward_claim_daily.json")["0299-日常任务奖励-日常-主页-试炼"]
...
def test_daily_reward_closes_when_no_unlocked_chest_remains() -> None:
    ...
    assert nodes["0268-日常任务奖励-领取-宝箱"]["on_error"] == ["0280-日常任务奖励-关闭"]
...
def test_stamina_purchase_click_targets_the_ten_cost_under_plus_eighty() -> None:
    ...
    node = nodes["0780-剑林凝结体体力-购买-体力-一次"]
    assert node["recognition"]["param"]["all_of"][-1] == ("0981-剑林凝结体体力-剑林-体力-价格10")
...
def test_jianlin_cleanup_closes_both_nested_pages_before_home_success() -> None:
    ...
    assert daily_close["custom_action_param"]["fixed_click_mode"] == ("jianlin_page_close")
...
def test_hero_claim_recognition_covers_visible_button_variants() -> None:
    node = _load("daily/hero_dispatch_daily.json")["0754-英雄派遣-英雄-领取-按钮"]
...
def test_dungeon_close_uses_the_visible_close_control_before_home_boundary() -> None:
    ...
    for name in ("0337-副本扫荡-成功-关闭",):
        assert nodes[name]["recognition"]["param"]["all_of"] == ["0374-副本扫荡-副本-关闭"]
...
def test_shadow_ruins_starts_battle_from_the_preparation_screen() -> None:
    ...
    assert start["next"] == ["[JumpBack]1509-MJA-影之遗迹地图推进-战斗等待"]
...
def test_shadow_ruins_confirms_the_exit_dialog() -> None:
    ...
    assert confirm["custom_action_param"]["fixed_click_mode"] == ("shadow_completion_confirm")
    assert confirm["recognition"]["param"]["all_of"] == ["1596-MJA-影之遗迹-退出确认-文案"]
    assert confirm["next"] == [
    ...
    assert reward["recognition"]["param"]["all_of"] == ["1598-MJA-影之遗迹-奖励弹窗文案"]
    assert nodes["1598-MJA-影之遗迹-奖励弹窗文案"]["expected"] == [
        "^恭$",
        "^喜获得$",
        "^[恭泰]喜获得$",
    ]
    assert reward["custom_action_param"]["action_id"] == ("dismiss_shadow_reward_popup")
    assert reward["custom_action_param"]["fixed_click_mode"] == ("shadow_reward_blank")
    assert reward["next"] == [
        "1173-影之遗迹-选择-进行中",
        "影之遗迹-本期关卡均已击破",
        "[JumpBack]1170-影之遗迹-打开-画卷",
    ...
    for relative, node_names in cases.items():
        ...
        for node_name in node_names:
            expected_next = (
                ...
                if (
                    relative == "daily/jianlin_resource_condensate_stamina_daily.json"
...
def test_shadow_ruins_keeps_three_fixed_foreground_clicks() -> None:
    node = _load("daily/shadow_ruins_daily.json")["1501-MJA-影之遗迹地图推进-前景三点循环"]
...
def test_retired_break_array_is_not_enabled_by_the_daily_preset() -> None:

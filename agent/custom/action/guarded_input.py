...
def _record_denial(context: Any, params: Mapping[str, Any], reason: str) -> None:
    del context
    _LOGGER.warning(
        "guarded input denied task=%s action=%s reason=%s",
        params.get("task_id", ""),
...
_FIXED_CLICK_MODES: Mapping[tuple[str, str], tuple[int, int, int, int]] = {
    ...
    # than the adjacent home marker.  The home page still has to be present
    # in the same-frame GuardedInput evidence before this click is allowed.
    (
        "open_trial_sword",
        "trial_entry_button",
    ): (1054, 540, 49, 21),
    # The updated top navigation is a Unity control that intermittently
    # rejects a zero-duration click while the home HUD is settling. The
    # pipeline still supplies same-frame OCR evidence; this named mode only
    # changes the input gesture to a short drag-tap on the calibrated icon.
    (
        "open_dungeon",
        "dungeon_button",
    ): (1025, 5, 65, 65),
    # The Yanwangling master sweep quantity plus is a stable Unity control.
    # The pipeline proves the sweep panel, selected dungeon card, and master
    # difficulty in one frame before every click; this named box avoids making
    # ticket OCR part of the fixed twenty-click business rule.
    (
        "assign_sweep_ticket",
    ...
    # The current shadow stage opens on the battle-preparation screen. The
    # “开战” control is a large lower-right button and is not reliably
    # represented by the OCR box; the preparation-page OCR must still gate
    # this named fixed tap.
    (
        "challenge_shadow_stage",
        "shadow_battle_start",
    ): (1090, 540, 180, 170),
    # The shadow ruins page is opened inside the painting surface and uses the
    # same calibrated upper-right close icon.  The page marker and template
    # recognition remain mandatory evidence before this cleanup tap.
    (
        "close_shadow_page",
        "shadow_page_close",
    ): (1190, 20, 44, 44),
    ...
    # The archived r22 victory card has no confirm/continue footer. Its
    # same-frame victory title and game-brand anchors gate this historically
    # verified lower-right blank-area dismissal box.
    (
        "dismiss_break_array_result",
        "break_array_victory_blank",
    ): (1040, 600, 160, 70),
    # The 720p activity surface uses a round crossed-swords close control at
    # the upper-right.  The task-specific page recognizer gates this small,
    # screenshot-verified hit box; the generic modal-close template is a
    # different visual asset and does not match this control reliably.
    (
        "close_break_array_page",
        "break_array_page_close",
    ): (1200, 30, 25, 25),
    (
        "close_break_array_activity",
    ...
    # The 50-soul-jade offer is deliberately skipped. Android BACK does not
    # dismiss this Unity modal, so use the already calibrated page-close
    # anchor as a modal-outside tap. Same-frame purchase-dialog and price-50
    # evidence keeps this path away from both consumptive purchase buttons.
    (
        "dismiss_jianlin_stamina_purchase",
        "jianlin_page_close",
    ): (1205, 33, 18, 18),
    # Closing a Jianlin resource detail page reveals the parent 剑林/养成
    # selector, which has the same calibrated upper-right X. Keep the second
    # layer under its existing independent cleanup action budget.
    (
        "close_daily_tasks",
        "jianlin_page_close",
    ): (1205, 33, 18, 18),
    ...
    # The collection page asks for an explicit confirmation before deploying
    # all detected collection points. The dialog OCR gates this small fixed
    # button area; it is not a generic confirmation click.
    (
        "confirm_collection_deployment",
        "collection_deployment_confirm",
    ): (820, 460, 120, 80),
    ...
    def run(self, context: Any, argv: CustomAction.RunArg) -> bool:
        ...
        try:
            ...
            if params is not None:
                _record_denial(context, params, f"{type(exc).__name__}: {exc}")
            return False
